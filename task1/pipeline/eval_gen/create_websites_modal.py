...
def generate_one_site_remote(args: dict) -> dict:
    ...
    try:
        ...
        if args.get("package", True):
            ...
            tasks_root = Path(VOLUME_MOUNT_PATH) / "tasks"
...
@app.local_entrypoint()
def download(target_dir: str = "", what: str = "both"):
    ...
    if what in ("sites", "both"):
        ...
        print(f">>> downloading reference_sites/ -> {local_ref}")
        n = _download_volume_subtree("/reference_sites", local_ref)
        total += n
        print(f"    {n} file(s)")

    if what in ("tasks", "both"):
        local_tasks = base / "tasks"
        print(f">>> downloading tasks/ -> {local_tasks}")
        n = _download_volume_subtree("/tasks", local_tasks)
        total += n
        print(f"    {n} file(s)")
