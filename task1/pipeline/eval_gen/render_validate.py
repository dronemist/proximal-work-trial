...
def static_html_checks(html: str, issues: list[str], css: str = "", spec=None) -> None:
    ...
    """
    # Allow empty <script> tags (some frameworks ship them) but flag non-empty.
...
def validate_library_css(
    ...
    # Cheap static scan first — no need to spin up Playwright if @keyframes
    # or external-URL violations are visible in the source.
...
def render_and_validate(
    *,
    ...
    static_html_checks(html_text, res.issues, css=css_text, spec=spec)
