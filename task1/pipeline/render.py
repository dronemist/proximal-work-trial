...
# Dump every visible element's tag + 3 style fields. Kept deliberately minimal —
# the DOM/palette/typography metrics only need these. Add fields when a metric
# actually needs them.
_DOM_DUMP_JS = r"""
() => {
  const out = [];
...
def render(
    ...
    When `fail_on_off_origin=True` and off-origin requests are detected,
    sets meta["off_origin_violation"] = True (does not raise).
    """
    if "://" not in source:
    ...
    with sync_playwright() as p:
        ...
        try:
            # Wrap in arrow fn so Playwright awaits the returned Promise.
