...
**Multi-viewport grading needed its own axis.** The structured metrics (palette, text, SSIM, block_match) don't directly measure whether a layout reflows — a non-responsive page can still score well on each of them. We added the **overflow** metric specifically to catch this, applied as a multiplier so a clearly-broken responsive layout halves the page composite.
...
7. **Overflow promoted to partial multiplier.** Initially overflow was a weighted metric (weight 0.18), then a soft ceiling at `0.5 + 0.5 × overflow_score`. Eyeball calibration on broken-responsive mobile pages (e.g. 008-hr-payroll/time-off mobile at 588px on a 375px viewport, missing widgets) showed the 0.5 floor was too generous. Final form: `composite = base × (0.5 + 0.5 × overflow_score)` — overflow=0 now halves the composite instead of flooring at 0.5.
