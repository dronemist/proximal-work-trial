...
_VLM_ANIMATION_PROMPT = """You are evaluating whether a candidate webpage's animations faithfully replicate a reference webpage's animations. Each image is a filmstrip showing animation frames across time (left-to-right, top-to-bottom), with timestamps labeled on each frame.

The first image is the REFERENCE filmstrip (what the animations should look like).
The second image is the CANDIDATE filmstrip (what the AI agent produced).

Score from 0.0 to 1.0 by checking:

1. **Animation presence** — Does the candidate have animations at all? No animation = 0.0.
2. **Motion direction & type** — Do elements move/fade/scale in the same direction and manner? A fade-up vs fade-down is wrong even if both fade.
3. **Timing & pacing** — Do animations start and end at roughly the same times? Is the easing similar (snappy vs gradual)?
4. **Stagger pattern** — If reference staggers elements (cards appearing one-by-one), does the candidate do the same?
5. **Visual end state** — Does the final frame match? Elements should end in the same positions/opacities.

Scoring scale:
  1.0 = animations are identical in type, direction, timing, and feel
  0.8 = same animations, minor timing differences
  0.6 = right type of animation but wrong direction or significantly different timing
  0.4 = some animation present but largely different from reference
  0.2 = minimal/wrong animations
  0.0 = no animations or completely unrelated motion

Respond ONLY with this JSON, nothing else:
{"score": 0.0, "reason": "one sentence about the most important animation difference"}"""


def judge_animation(reference_filmstrip: Path, candidate_filmstrip: Path) -> MetricResult:
    api_key = os.environ.get("ANTHROPIC_API_KEY")
    if not api_key:
        return MetricResult(name="vlm_animation", score=None,
                            extra={"skipped": True, "reason": "ANTHROPIC_API_KEY not set"})
    try:
        import anthropic
    except ImportError as e:
    ...
    try:
        resp = client.messages.create(
            ...
            messages=[{
                "role": "user",
                "content": [
                    {"type": "text", "text": "REFERENCE ANIMATION FILMSTRIP:"},
                    {"type": "image", "source": {"type": "base64", "media_type": "image/png",
                                                 "data": _png_b64(reference_filmstrip)}},
                    {"type": "text", "text": "CANDIDATE ANIMATION FILMSTRIP:"},
                    {"type": "image", "source": {"type": "base64", "media_type": "image/png",
                                                 "data": _png_b64(candidate_filmstrip)}},
                    {"type": "text", "text": _VLM_ANIMATION_PROMPT},
                ],
            }],
        )
    except Exception as e:
    ...
    try:
        ...
        return MetricResult(name="vlm_animation", score=None,
                            extra={"error": "non-numeric score", "raw": text[:500]})
    score = max(0.0, min(1.0, score))
    return MetricResult(name="vlm_animation", score=score,
                        extra={"reason": parsed.get("reason", "")[:300],
                               "model": VLM_MODEL,
                               "wall_s": round(time.time() - started, 2)})
