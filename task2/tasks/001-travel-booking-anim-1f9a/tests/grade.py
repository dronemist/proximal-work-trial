...
def _capture_agent_animation_keyframes(
    ...
    # Second pass: video capture (needs its own context with record_video_dir)
    ...
    with sync_playwright() as p:
        ...
        try:
            for html_path in sorted(agent_html_dir.glob("*.html")):
                ...
                for vp_name, (w, h) in VIEWPORTS.items():
                    key = f"{stem}.{vp_name}"
                    if key not in results or not results[key].get("metadata"):
                        continue
                    ctx = browser.new_context(
                        viewport={"width": w, "height": h},
                        device_scale_factor=1,
                    ...
                    ctx.close()
                    if video_tmp and Path(video_tmp).exists():
                        final = videos_dir / f"{stem}.{vp_name}.webm"
                        Path(video_tmp).rename(final)
                        results[key]["video"] = str(final)
        finally:
            browser.close()
