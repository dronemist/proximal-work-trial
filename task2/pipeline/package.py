"""
Scaffold + render Harbor tasks.

Two modes:

1. **Local rendering** (`package_one_local`): scaffolds one site and renders
   PNGs using the local Playwright install.  Used by create_websites_modal.py
   inside the Modal container (which already has Chromium + fonts).

2. **Batch via Modal** (CLI / `package_sites`): scaffolds N sites locally,
   then renders all PNGs in a single Modal session.
...
def package_one_local(site_dir: Path, output_root: Path) -> Path:
    """Scaffold + render one site using local Playwright. No Modal needed.

    Returns the task directory path. Requires Playwright + Chromium to be
    installed in the current environment (true inside Modal containers and
    the verifier image).
    """
    from render import render as render_page

    name = site_dir.name
    task_dir = scaffold(name, source_dir=site_dir.parent, output_root=output_root)
    ...
    for html in htmls:
        for vp_name, (w, h) in VIEWPORTS.items():
            out_name = f"{html.stem}.{vp_name}.png"
            ver_png = ver_ref / out_name
            meta = render_page(
                source=str(html),
                output_png=ver_png,
                viewport_width=w,
            ...
            chromium_version = meta.get("chromium_version", chromium_version)
            shutil.copy2(ver_png, env_ref / out_name)
            print(f"    {out_name}  ({ver_png.stat().st_size:,} bytes)")

    manifest = {
        "source_hashes": _hash_source(site_dir),
    ...
    (task_dir / "tests" / "build_manifest.json").write_text(
        json.dumps(manifest, indent=2)
    )
    print(f"[package_one_local] {name}: {len(htmls)} page(s), "
          f"{len(htmls) * len(VIEWPORTS)} screenshot(s) → {task_dir}")
    return task_dir
