...
def _generate_one(spec: BrandSpec, site_work_dir: Path, cfg) -> dict:
    ...
    for lib_attempt in range(MAX_LIBRARY_RETRIES + 1):
        ...
        lib_issues = validate_library_css(
            lib_out["css"], site_work_dir, cfg,
            animated=bool(spec.animation_spec),
        )
        if not lib_issues:
            break
...
def cmd_generate(cfg) -> int:
    ...
    for i in range(cfg.n_tasks):
        ...
        if cfg.animation_mode != "none":
            resolve_animation_spec(spec, rng, complexity=cfg.animation_complexity)
            print(f"  animation: {cfg.animation_complexity} tier, "
                  f"{len(spec.animation_spec.get('entrance_animations', []))} entrance, "
                  f"{len(spec.animation_spec.get('ambient_animations', []))} ambient")
