...
- **CSS animations are REQUIRED.** Include `@keyframes` rules and animation
  utility classes as specified in the animation spec below. All animations
  must be pure CSS — no JavaScript. Define the keyframes and animation classes
  in this stylesheet so every page can reference them consistently.
- `:hover` / `:focus` transitions are also allowed."""
...
def make_user_prompt(spec: BrandSpec, brief: str) -> str:
    from pipeline.eval_gen.spec_library.animations import (
        collect_keyframes_css,
        format_animation_spec_for_prompt,
    )
...
ANIMATION SPEC — include these @keyframes and animation utility classes in the
stylesheet. Every page will reference these classes to apply entrance/ambient
animations consistently.

{format_animation_spec_for_prompt(spec.animation_spec)}

REQUIRED @keyframes (include these verbatim, then add `.animate-<name>` utility
classes that apply them with the specified timing):

{collect_keyframes_css(spec.animation_spec)}
"""
    return f"""\
